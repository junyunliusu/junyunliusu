...
Node getRandomEquation() {
	...
	for (auto id : priority)//按优先级合并，有点乱
	{
		...
		if (getPriority(lop) < getPriority(tree[id].op) && getPriority(tree[id].op) >= getPriority(rop))//优先级比较
	...
	if (vis.find(ans.checkeq) != vis.end())//进行查重，确保没有重复
		return { Fraction(),"","","",0,0 };
